        ...
        <v-card-text>
          ...
          <v-layout row wrap v-if="is_limit_by_range">
            <v-flex xs12 sm12 md6>
              ...
              <v-date-picker :landscape="true" v-model="from" :max="min_date">
              </v-date-picker>
            </v-flex>
            <v-flex xs12 sm12 md6>
    ...
    async generatePDF(body, masterData) {
      ...
      if (masterData.status == 1) {
        dd.content[1].text = dd.content[1].text.concat(
          " for active subcategories"
        );
      }
      if (masterData.status == 0) {
        dd.content[1].text = dd.content[1].text.concat(
          " for inactive subcategories"
        );
      }
      if (masterData.is_limit_by_range) {
        dd.content[1].text = dd.content[1].text.concat(
          ` subcategories created from : ${masterData.from} to : ${masterData.to}`
        );
      }
